...
def individual_token_status(response):
    ...
    token = token.decode("ascii")
    # group_token_request(("150.164.213.243", 51001), student_id, nonce, token)
    print("Status da messagem: ")
    print(message_type, student_id, nonce, token, status)
    return message_type, student_id, nonce, token, status


def group_token_request(server_address, student_id, nonce, token):
    print(
        "_______________________________Group Token Request__________________________________________"
    )
...
def group_token_response(response):
    print(
        "____________________________Group Token Response_____________________________________________"
    )
    # Formato ajustado: "H12sI64s", totalizando 82 bytes
